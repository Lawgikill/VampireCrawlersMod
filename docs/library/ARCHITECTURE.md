...
- Load `/api/card-map` and `/api/card-names` once on startup and after local data rebuild.
...
Gem IDs stay raw in save data, but the frontend formats them for tags by
stripping `GemConfig_`, splitting camel-case words, and rendering mana modifier
gems as `Mana +N` / `Mana -N`. For example, `GemConfig_YinYang` displays as
`Yin Yang`.
