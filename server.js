...
function normalizeCard(card, pileId, index, cardCosts = {}, context = {}) {
  ...
  return {
    ...
    crackState: card.BreakableCrackState || card.crackState || "",
    breakableCrackStage: card.BreakableCrackStage || card.breakableCrackStage || 0,
    timesPlayedThisTurn: card.BreakableTimesPlayedThisTurn || card.timesPlayedThisTurn || 0,
    cardCrackSprite: card.CardCrackSprite || card.cardCrackSprite || "",
    comboCostHighlighted: Boolean(card.IsComboCostHighlighted ?? card.comboCostHighlighted),
    comboCostText: card.ComboCostText || card.comboCostText || "",
    comboDiagnostics: card.ComboDiagnostics || card.comboDiagnostics || {},
    comboComponents: card.ComboComponents || card.comboComponents || [],
...
function buildSnapshot({ source, sourcePath, lastModified, pileNodes, cardCosts, context, profileId }) {
  ...
  return {
    ...
    isInCombat: typeof context.liveState?.IsInCombat === "boolean" ? context.liveState.IsInCombat : null,
    isPlayerTurn: typeof context.liveState?.IsPlayerTurn === "boolean" ? context.liveState.IsPlayerTurn : null,
    enemiesRemaining: Number.isFinite(context.liveState?.EnemiesRemaining) ? context.liveState.EnemiesRemaining : null,
    currentMana: Number.isFinite(context.liveState?.CurrentMana) ? context.liveState.CurrentMana : null,
    cachedMana: Number.isFinite(context.liveState?.CachedMana) ? context.liveState.CachedMana : null,
    startingMana: Number.isFinite(context.liveState?.StartingMana) ? context.liveState.StartingMana : null,
    fccMana: Number.isFinite(context.liveState?.FccMana) ? context.liveState.FccMana : null,
    displayedMana: Number.isFinite(context.liveState?.DisplayedMana) ? context.liveState.DisplayedMana : null,
    manaDiagnostics: context.liveState?.ManaDiagnostics || {},
    gameStateName: context.liveState?.GameStateName || "",
    combatDiagnostics: context.liveState?.CombatDiagnostics || {},
    combatComponents: context.liveState?.CombatComponents || [],
